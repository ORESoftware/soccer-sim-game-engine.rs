...
impl PlayerAgent {
    ...
    fn learned_stale_dribble_release_override(
        ...
        // Only exempt the elite when there is genuine room to attack — AND no
        // clearly-open advanced outlet. We only reach this branch under real
        // pressure (the danger gate above), where an open forward runner is a
        // better ball than dribbling on; the elite yields the pass instead of
        // carrying into the tackle. (Falls through to the floor-pass release
        // below, which is guaranteed viable since overall openness >= the open
        // forward openness here.)
        let _open_forward_outlet = observation.visible_forward_pass_options > 0
