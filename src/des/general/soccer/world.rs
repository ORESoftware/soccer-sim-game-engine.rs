...
impl SoccerMatch {
    ...
    pub(crate) fn record_recent_turnover_penalties(&mut self, losing_team: Team) {
        ...
        // Respect the turnover-penalty kill switch shared with `penalize_turnover_window`,
        // and de-duplicate against it: that path (interception/dispossession sites) and
        // this one (the open-play possession-flip detector + out-of-bounds) both stamp
        // `last_turnover_penalty_tick`, so a single turnover is penalized at most once per
        // tick by whichever fires first — never double-counted.
        ...
        self.last_turnover_penalty_tick = Some(self.tick);
        let window_ticks = ((TURNOVER_HISTORY_WINDOW_SECONDS / self.config.dt_seconds.max(1e-6))
            .round() as u64)
            .max(1);
        let now = self.tick;
        let recent = self
